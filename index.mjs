...
async function realIpfs(arrayBuffer) {
  const result = execSync(`docker run -v ${process.cwd()}:/opt/ipfs-test ipfs/go-ipfs add -q --offline --only-hash /opt/ipfs-test/file --cid-version 1`)
  const cid = /(bafy.+)/.exec(result.toString())[0]
  return cid.toString()
}

import * as Hash from 'ipfs-only-hash'
Hash.of(fs.readFileSync('./file'), {cidVersion: 1}).then($ => {
...
multiformatHash(fs.readFileSync('./file')).then($ => {
...
ipfsHash(fs.readFileSync('./file')).then($ => {
...
realIpfs(fs.readFileSync('./file')).then($ => {
